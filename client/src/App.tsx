      ...
      content: (
         <>
            ...
            <ul className="list-disc pl-5 space-y-2 text-ink-light mb-6">
               <li><strong><Link to="/services/assessments" className="text-ida hover:underline">Medication Reviews</Link>:</strong> Bring in your current cabinet contents. We'll verify what is safe to use together.</li>
               ...
               <li><strong>Flavoring:</strong> If they refuse the medicine, we can <Link to="/services/compounding" className="text-ida hover:underline">compound it</Link> with a flavor they actually like.</li>
            </ul>
      ...
      content: (
         <>
            ...
            <ul className="list-disc pl-5 space-y-2 text-ink-light mb-6">
               ...
               <li><strong><Link to="/services/delivery" className="text-ida hover:underline">Free Delivery</Link>:</strong> Once transferred, you qualify for our free local delivery immediately.</li>
            </ul>
         </>
      )
   },
...
// --- URL MAPPING for SEO-friendly routing ---
const VIEW_TO_URL: Record<string, string> = {
   HOME: '/',
   SERVICES_INDEX: '/services',
   SERVICE_DETAIL: '/services',
   RESOURCES_INDEX: '/resources',
   BLOG_INDEX: '/resources/articles',
   BLOG_POST: '/resources/articles',
   RESOURCES_FORMS: '/resources/forms',
   RESOURCES_FAQS: '/resources/faq',
   RESOURCES_GLOSSARY: '/resources/glossary',
   ABOUT: '/about',
...
const SERVICE_SLUGS: Record<string, string> = {
   ...
   'kids-meds-vaccines': 'kids',
   'vaccinations-travel': 'vaccines',
   'assessments-monitoring': 'assessments',
   'medication-customization': 'compounding',
   'health-advice-support': 'health-advice',
   'free-delivery': 'delivery',
};

// Reverse lookup: slug → service id
const SLUG_TO_SERVICE_ID: Record<string, string> = Object.entries(SERVICE_SLUGS).reduce(
   (acc, [id, slug]) => ({ ...acc, [slug]: id }), {} as Record<string, string>
);

const ORGANIZATION_SCHEMA = {
   "@type": "Pharmacy",
   "name": "Golf Links IDA Pharmacy",
   "url": "https://golflinkspharmacy.ca",
   "telephone": "+1-289-239-8000",
   "priceRange": "$$",
   "address": {
      "@type": "PostalAddress",
      "streetAddress": "26 Legend Ct Unit 3",
...
const ResourcesFooterCTA = () => (
   <div className="flex flex-col md:flex-row items-center justify-center gap-4 text-center border-t border-stone-200 pt-12 mt-16">
      ...
      <p className="text-sm">
         Call us at <a href="tel:2892398000" className="text-ida font-medium hover:underline">(289) 239-8000</a> or <Link to="/#transfer" className="text-ida font-medium hover:underline">move your prescriptions in a few clicks →</Link>
      </p>
   </div>
);
...
const StatusCard = ({ onNavigate, reviewStats }: { onNavigate: (slug: string) => void, reviewStats: GoogleReviewStats }) => (
   <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm relative overflow-hidden h-full flex flex-col">
      ...
      <div className="flex justify-between items-start mb-6">
         <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               ...
               <span className="text-sm font-medium text-green-700">Open · Closes 6:30 pm</span>
            </div>
         </div>
      ...
      <Link
         to={`/resources/articles/${BLOG_POSTS[0].id}`}
         className="mt-auto -mx-6 -mb-6 p-5 rounded-b-2xl cursor-pointer group
          ...
          transition-all duration-200 ease-out flex flex-col no-underline"
      >
...
const ResourcesIndexView = () => {
   const navigate = useNavigate();
   return (
      <div className="pt-24 md:pt-32 pb-16 min-h-screen">
         <SEOHead title="Patient Resources | Golf Links IDA Pharmacy Ancaster" description="Helpful tools, forms, articles, and guides to manage your family's health. Golf Links IDA Pharmacy in Ancaster." />
         <Section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
               <Card title="Articles & News" icon={<BookOpen className="w-6 h-6" />} actionLabel="Read articles" onClick={() => navigate('/resources/articles')}>
                  Expert advice on common ailments, medication safety, and seasonal health tips from our pharmacists.
               </Card>
               <Card title="Patient Forms" icon={<FileText className="w-6 h-6" />} actionLabel="View forms" onClick={() => navigate('/resources/forms')}>
                  Downloadable forms for prescription transfers, delivery authorization, and more. Save time by filling them out at home.
               </Card>
               <Card title="Common Questions" icon={<HelpCircle className="w-6 h-6" />} actionLabel="Get answers" onClick={() => navigate('/resources/faq')}>
                  Answers to frequently asked questions about transfers, insurance, delivery, and payments.
               </Card>
               <Card title="Pharmacy Glossary" icon={<Book className="w-6 h-6" />} actionLabel="Learn terms" onClick={() => navigate('/resources/glossary')}>
...
const BlogIndex = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="Health Articles | Golf Links IDA Pharmacy Ancaster" description="Practical health advice for parents and families from our pharmacists in Ancaster. Topics include medications, allergies, and kids' health." />
      <Section>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {BLOG_POSTS.map((post) => (
               <Link key={post.id} to={`/resources/articles/${post.id}`} className="group cursor-pointer flex flex-col h-full no-underline">
...
const BlogPostView = () => {
   ...
   if (!post) return <div className="pt-24 md:pt-32 pb-16 min-h-screen text-center"><p>Article not found.</p></div>;

   return (
      <div className="pt-24 md:pt-32 pb-16 min-h-screen bg-white">
         <SEOHead title={`${post.title} | Golf Links IDA Pharmacy`} description={post.subtitle} />
         <Schema
            data={{
               "@type": "Article",
               "headline": post.title,
               ...
               "datePublished": "2023-10-01", // Placeholder, ideally should be in post object
               "dateModified": "2023-10-01"
            }}
            scriptId="schema-article"
         />
         <article className="max-w-3xl mx-auto px-6">
            <Link to="/resources/articles" className="flex items-center text-sm text-ink-light hover:text-ida mb-8 transition-colors group no-underline">
...
const FormsView = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="Patient Forms | Golf Links IDA Pharmacy Ancaster" description="Download prescription transfer, delivery authorization, MedsCheck consent, and flu shot forms. Golf Links IDA Pharmacy in Ancaster." />
      <Section>
...
const FaqView = () => {
   ...
   return (
      <div className="pt-24 md:pt-32 pb-16 min-h-screen">
         <SEOHead title="FAQs | Golf Links IDA Pharmacy Ancaster" description="Frequently asked questions about prescription transfers, free delivery, insurance, and pharmacy services at Golf Links IDA Pharmacy." />
         <Schema data={ORGANIZATION_SCHEMA} scriptId="schema-org" />
         <Schema
            data={{
               "@type": "FAQPage",
         ...
         />
         <Section>
            <Breadcrumbs items={[{ label: 'Resources', href: '/resources' }, { label: 'FAQs' }]} />
            <div className="mb-12">
...
const GlossaryView = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="Pharmacy Glossary | Golf Links IDA Pharmacy Ancaster" description="Understanding common pharmacy terms like compounding, co-pay, ODB, and MedsCheck. Plain-English definitions from Golf Links IDA Pharmacy." />
      <Section>
...
const ServicesIndexView = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="Pharmacy Services | Golf Links IDA Pharmacy Ancaster" description="Compounding, free delivery, vaccinations, kids' health, prescription transfers and more. Full pharmacy services at Golf Links IDA Pharmacy Ancaster." />
      <Section>
         <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            {ALL_SERVICES_DATA.map((service) => {
               ...
               return (
                  <Link
                     key={service.id}
                     to={`/services/${slug}`}
                     className="bg-white border border-stone-200 rounded-xl p-6 flex flex-col h-full hover:shadow-md transition-shadow cursor-pointer no-underline text-inherit"
...
const ServiceDetailView = () => {
   const { slug } = useParams<{ slug: string }>();
   const navigate = useNavigate();
   const serviceId = slug ? (SLUG_TO_SERVICE_ID[slug] || slug) : '';
   const service = ALL_SERVICES_DATA.find(s => s.id === serviceId);

   if (!service) return <div className="pt-24 md:pt-32 pb-16 min-h-screen text-center"><p>Service not found.</p></div>;

   const Icon = service.icon;

   return (
      <div className="pt-24 md:pt-32 pb-16 min-h-screen bg-white">
         <SEOHead title={`${service.title} | Golf Links IDA Pharmacy Ancaster`} description={service.fullDescription || service.shortDescription} />
         <Schema
            data={{
               "@type": "Service",
               "name": service.title,
               ...
               "termsOfService": "https://golflinkspharmacy.ca/privacy",
               "areaServed": {
                  "@type": "City",
                  "name": "Ancaster"
               }
            }}
            scriptId="schema-service"
         />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20">
               ...
               <div className="lg:col-span-7">
                  ...
                  <div className="flex gap-4">
                     <Button variant="primary" onClick={() => navigate('/#transfer')}>Contact us about this</Button>
                     <Link to="/services" className="no-underline"><Button variant="outline">Back to services</Button></Link>
                  </div>
               </div>
                  ...
                  <div className="sticky top-32">
                     ...
                     <div className="space-y-4">
                        {service.relatedServices.map(relatedId => {
                           ...
                           const relatedSlug = SERVICE_SLUGS[related.id] || related.id;
                           return (
                              <Link key={related.id} to={`/services/${relatedSlug}`} className="bg-white border border-stone-200 rounded-lg p-4 flex items-center gap-4 cursor-pointer hover:border-ida/50 transition-colors shadow-sm no-underline text-inherit block">
...
const AboutView = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="About Us | Golf Links IDA Pharmacy Ancaster" description="Meet Harsh, your pharmacist owner, and learn about our philosophy of empathy-first, custom care at Golf Links IDA Pharmacy in Ancaster." />
      <Section>
...
const PrivacyPolicyView = () => (
   <div className="pt-24 md:pt-32 pb-16 min-h-screen">
      <SEOHead title="Privacy Policy | Golf Links IDA Pharmacy Ancaster" description="How we collect, use, and safeguard your personal health information at Golf Links IDA Pharmacy in Ancaster, Ontario." />
      <Section>
         <Link to="/" className="flex items-center text-sm text-ink-light hover:text-ida mb-8 transition-colors group no-underline">
   ...
   <footer className="bg-ida text-white py-16 px-6 md:px-12 mt-auto relative isolate">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
         ...
         <div>
            ...
            <ul className="space-y-3 text-sm text-ida-faint opacity-80">
               <li><Link to="/services" className="hover:text-white transition-colors text-left font-medium no-underline text-inherit">Services Overview</Link></li>
               <li><Link to="/services/prescriptions" className="hover:text-white transition-colors text-left no-underline text-inherit">Everyday Prescriptions</Link></li>
               <li><Link to="/services/kids" className="hover:text-white transition-colors text-left no-underline text-inherit">Kids' Health & Vaccines</Link></li>
               <li><Link to="/services/compounding" className="hover:text-white transition-colors text-left no-underline text-inherit">Compounding & Custom Meds</Link></li>
               <li><Link to="/services/assessments" className="hover:text-white transition-colors text-left no-underline text-inherit">Assessments & Reviews</Link></li>
               <li><Link to="/services/vaccines" className="hover:text-white transition-colors text-left no-underline text-inherit">Travel Health</Link></li>
            </ul>
         </div>
         ...
         <div>
            ...
            <ul className="space-y-3 text-sm text-ida-faint opacity-80">
               <li><Link to="/resources" className="hover:text-white transition-colors text-left font-medium no-underline text-inherit">Resources Hub</Link></li>
               <li><Link to="/resources/articles" className="hover:text-white transition-colors text-left no-underline text-inherit">Articles & News</Link></li>
               <li><Link to="/resources/forms" className="hover:text-white transition-colors text-left no-underline text-inherit">Patient Forms</Link></li>
               <li><Link to="/resources/faq" className="hover:text-white transition-colors text-left no-underline text-inherit">Common Questions</Link></li>
               <li><Link to="/resources/glossary" className="hover:text-white transition-colors text-left no-underline text-inherit">Pharmacy Glossary</Link></li>
            </ul>
         </div>
         ...
         <div>
            ...
            <ul className="space-y-2 text-sm text-ida-faint opacity-80">
               <li className="flex justify-between border-b border-ida-light/20 pb-2"><span>Mon - Fri</span> <span>9am - 6:30pm</span></li>
               <li className="flex justify-between border-b border-ida-light/20 pb-2"><span>Saturday</span> <span>Closed</span></li>
            ...
            <div className="mt-6 pt-6 border-t border-ida-light/20">
               <Link to="/#transfer" className="text-white hover:underline text-sm font-medium flex items-center no-underline">
      ...
      <div className="max-w-6xl mx-auto mt-20 pt-8 border-t border-ida-light/30 flex flex-col md:flex-row justify-between items-center text-xs text-ida-faint opacity-60">
         ...
         <div className="flex gap-6 mt-4 md:mt-0">
            <Link to="/" className="no-underline text-inherit hover:text-white">Home</Link>
            <Link to="/about" className="no-underline text-inherit hover:text-white">About Us</Link>
            <Link to="/#contact" className="no-underline text-inherit hover:text-white">Contact</Link>
            <Link to="/privacy" className="no-underline text-inherit hover:text-white">Privacy Policy</Link>
         </div>
      </div>
   </footer>
);
...
const App: React.FC = () => {
   ...
   // Handle hash-based scrolling (e.g. /#transfer, /#contact)
   useEffect(() => {
      if (location.hash) {
      ...
      <div className="min-h-screen bg-paper font-sans text-ink selection:bg-ida/10 selection:text-ida flex flex-col w-full overflow-x-clip">
         ...
         <nav className="fixed top-0 w-full bg-paper/80 border-b border-stone-200 z-50 backdrop-blur-md">
            <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
               <Link to="/" className="flex items-center gap-3 group no-underline" onClick={closeMenu}>
               ...
               <div className="hidden md:flex items-center gap-8 text-sm font-medium text-ink-light">
                  <div className="flex items-center gap-6">
                     <Link to="/services" className={`transition-colors no-underline ${location.pathname.startsWith('/services') ? 'text-ida font-semibold' : 'text-ink-light hover:text-ida'}`}>Services</Link>

                     {/* Resources Dropdown */}
                     <div className="relative group">
                        <Link
                           to="/resources"
                           className={`flex items-center gap-1 transition-colors py-4 no-underline ${location.pathname.startsWith('/resources') ? 'text-ida font-semibold' : 'text-ink-light hover:text-ida'}`}
                        >
                        ...
                        <div className="absolute top-[3.5rem] left-0 w-48 bg-white border border-stone-100 rounded-xl shadow-soft p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 transform origin-top-left z-50 flex flex-col">
                           <Link to="/resources/articles" className="text-left px-4 py-2 text-sm text-ink hover:bg-stone-50 rounded-lg transition-colors no-underline">Articles</Link>
                           <Link to="/resources/forms" className="text-left px-4 py-2 text-sm text-ink hover:bg-stone-50 rounded-lg transition-colors no-underline">Forms</Link>
                           <Link to="/resources/faq" className="text-left px-4 py-2 text-sm text-ink hover:bg-stone-50 rounded-lg transition-colors no-underline">FAQs</Link>
                           <Link to="/resources/glossary" className="text-left px-4 py-2 text-sm text-ink hover:bg-stone-50 rounded-lg transition-colors no-underline">Glossary</Link>
                        </div>
                     </div>

                     <Link to="/services/delivery" className="hover:text-ida transition-colors no-underline text-ink-light">Free Delivery</Link>
                  </div>

                  <div className="h-4 w-px bg-stone-300"></div>

                  <div className="flex items-center gap-4">
                     <Link to="/#contact" className="hover:text-ida transition-colors no-underline text-ink-light">Contact</Link>
                     <Link to="/#transfer" className="text-ida font-medium hover:underline no-underline">Transfer</Link>
                     <Link to="/#transfer" className="no-underline"><Button variant="primary" className="py-2 px-4 text-xs shadow-none">Refill Now</Button></Link>
                  </div>
               </div>
            ...
            {isMenuOpen && (
               <div className="md:hidden bg-paper border-b border-stone-200 p-6 flex flex-col space-y-4 shadow-lg absolute w-full h-[calc(100vh-64px)] overflow-y-auto z-40">
                  <Link to="/services" className="text-lg font-medium text-left no-underline text-ink" onClick={closeMenu}>Services</Link>
                  <Link to="/services/delivery" className="text-lg font-medium text-left no-underline text-ink" onClick={closeMenu}>Free Delivery</Link>
                  ...
                  <div>
                     ...
                     {isResourcesOpen && (
                        <div className="flex flex-col pl-4 mt-2 space-y-2 border-l-2 border-stone-100 ml-1 bg-stone-50/50 py-2 rounded-r-lg">
                           <Link to="/resources" className="text-left text-ink-light py-2 px-2 hover:bg-stone-100 rounded font-medium no-underline" onClick={closeMenu}>Overview</Link>
                           <Link to="/resources/articles" className="text-left text-ink-light py-2 px-2 hover:bg-stone-100 rounded no-underline" onClick={closeMenu}>Articles</Link>
                           <Link to="/resources/forms" className="text-left text-ink-light py-2 px-2 hover:bg-stone-100 rounded no-underline" onClick={closeMenu}>Forms</Link>
                           <Link to="/resources/faq" className="text-left text-ink-light py-2 px-2 hover:bg-stone-100 rounded no-underline" onClick={closeMenu}>FAQs</Link>
                           <Link to="/resources/glossary" className="text-left text-ink-light py-2 px-2 hover:bg-stone-100 rounded no-underline" onClick={closeMenu}>Glossary</Link>
                        </div>
                     )}
                  </div>

                  <Link to="/#contact" className="text-lg font-medium text-left no-underline text-ink" onClick={closeMenu}>Contact</Link>

                  <div className="pt-6 mt-2 border-t border-stone-100 grid grid-cols-2 gap-4">
                     <Link to="/#transfer" className="no-underline" onClick={closeMenu}><Button variant="outline" fullWidth>Transfer</Button></Link>
                     <Link to="/#transfer" className="no-underline" onClick={closeMenu}><Button variant="primary" fullWidth>Refill Now</Button></Link>
                  </div>
               </div>
            )}
         </nav>
         ...
         <main className="flex-grow">
            <Routes>
               <Route path="/resources" element={<ResourcesIndexView />} />
               <Route path="/resources/articles" element={<BlogIndex />} />
               <Route path="/resources/articles/:slug" element={<BlogPostView />} />
               <Route path="/resources/forms" element={<FormsView />} />
               <Route path="/resources/faq" element={<FaqView />} />
               <Route path="/resources/glossary" element={<GlossaryView />} />
               <Route path="/services" element={<ServicesIndexView />} />
                           ...
                           <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 xl:gap-20 2xl:gap-24 items-start">
                              ...
                              <div className="lg:col-span-5 flex flex-col justify-center">
                                 ...
                                 <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl text-ink leading-[1.1] mb-5 md:mb-6 tracking-tight">
                                    Custom Compounding & Pharmacy Care in the Meadowlands of Ancaster
                                 </h1>
                                 ...
                                 <div className="hidden md:block bg-white/75 backdrop-blur-sm rounded-xl p-5 mb-10 max-w-md border border-white/50 shadow-sm">
                                    <p className="text-lg text-ink leading-relaxed">
                                       Integrated with <a href="https://www.ancasterpediatrics.com/" target="_blank" rel="noopener noreferrer" className="text-ida font-semibold hover:underline">Ancaster Central Children's Clinic</a>, we coordinate directly with doctors to handle prescriptions the right way. Free prescription delivery across Ancaster and Meadowlands, with personal service you won't get from big-box pharmacies.
                                    </p>
                                 </div>

                                 <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-8 md:mb-10">
                                    <Button variant="primary" className="px-6" onClick={() => navigate('/#transfer')}>Move my prescriptions</Button>
                                    <Button variant="outline" className="border-stone-300 text-ink hover:border-ida" onClick={() => navigate('/#contact')}>Refill an existing script</Button>
                                 </div>
                                 ...
                                 <div className="flex flex-col gap-4 border-l-2 border-stone-200 pl-6 py-1">
                                    ...
                                    <div className="flex items-center gap-3">
                                       ...
                                       <span className="text-sm font-semibold text-ink-light">Harsh Pharmacist</span>
                                    </div>
                                 </div>
                                 ...
                                 <div className="lg:hidden mt-10">
                                    <StatusCard
                                       onNavigate={(slug) => {
                                          navigate(`/resources/articles/${slug}`);
                                       }}
                        ...
                        <div className="max-w-6xl mx-auto px-6 md:px-12">
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                              ...
                              <div className="bg-white border border-stone-200 rounded-xl p-6 shadow-sm hidden md:flex flex-col h-full">
                                 <div className="flex items-center gap-2 mb-2 font-bold text-ink">
                                    <FlaskConical className="w-5 h-5 text-ida shrink-0" />
                                    <span>Custom Compounding Pharmacy</span>
                                 </div>
                                 <p className="text-sm text-ink-light leading-relaxed">Personalized medications made in-house—custom doses, forms, and ingredient options when standard prescriptions don’t fit.</p>
                              </div>
               ...
               `}</style>
                              {ALL_SERVICES_DATA.map((service, index) => {
                                 const Icon = service.icon;
                                 return (
                                    <div key={index} className="w-[300px] sm:w-[320px] md:w-[360px] snap-center flex-shrink-0 cursor-pointer" onClick={() => navigate(`/services/${SERVICE_SLUGS[service.id] || service.id}`)}>
                        ...
                        <div className="text-center mt-12">
                           <button onClick={() => navigate('/services')} className="text-ink-light hover:text-ida transition-colors border-b border-stone-300 hover:border-ida pb-0.5 text-sm">
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-12">
                           ...
                           <div className="flex-1 bg-white p-8 rounded-2xl border border-stone-200 shadow-sm w-full max-w-md">
                              ...
                              <form className="space-y-4" onSubmit={handleTransferSubmit} ref={transferFormRef}>
                                 <div>
                                    <label htmlFor="transfer-name" className="block text-xs font-bold uppercase tracking-wider text-ink-light mb-1">Full Name</label>
                                    <input id="transfer-name" type="text" className="w-full bg-stone-50 border border-stone-200 rounded p-2 focus:border-ida focus:ring-1 focus:ring-ida outline-none transition-colors" placeholder="Jane Doe" required />
                                 </div>
                                 <div>
                                    <label htmlFor="transfer-phone" className="block text-xs font-bold uppercase tracking-wider text-ink-light mb-1">Phone Number</label>
                                    <input id="transfer-phone" type="tel" className="w-full bg-stone-50 border border-stone-200 rounded p-2 focus:border-ida focus:ring-1 focus:ring-ida outline-none transition-colors" placeholder="(555) 123-4567" required />
                                 </div>
                                 <div>
                                    <label htmlFor="transfer-pharmacy" className="block text-xs font-bold uppercase tracking-wider text-ink-light mb-1">Current Pharmacy Name</label>
                                    <input id="transfer-pharmacy" type="text" className="w-full bg-stone-50 border border-stone-200 rounded p-2 focus:border-ida focus:ring-1 focus:ring-ida outline-none transition-colors" placeholder="e.g. Shoppers on Golf Links" required />
                                 </div>
                                    ...
                                    <table className="w-full text-sm">
                                       <tbody>
                                          <tr className="border-b border-stone-200/50">
                                             ...
                                             <td className="py-2 text-right">9:00 am – 6:30 pm</td>
                                          </tr>
                                          <tr className="border-b border-stone-200/50">
                                             <td className="py-2 font-medium text-stone-400">Saturday</td>
